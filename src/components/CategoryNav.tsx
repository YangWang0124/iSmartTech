...
export function CategoryNav() {
  return <nav className="category-nav catalogue-mega-nav" aria-label="Product categories"><div className="container category-nav__inner">
    ...
    <div className="mega-menu brand-menu">
      ...
      <div className="mega-menu__panel brand-menu__panel">{catalogueBrandGroups.map(group => <section key={group.title}><strong>{group.title}</strong><div className="brand-menu__items">{group.brands.map(brand => <BrandMenuLink key={brand.label} brand={brand} onClick={closeMenu} />)}</div></section>)}</div>
    </div>
