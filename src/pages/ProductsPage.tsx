...
export function ProductsPage() {
  ...
  useEffect(() => {
    const query = window.matchMedia("(min-width: 443px) and (max-width: 970px)");
    const syncView = () => {
      setListDisabled(query.matches);
      if (query.matches) setView("grid");
    };
    syncView();
  ...
  return <main className="page container catalogue-page">
    ...
    {!categorySlug || categoryEntry?.category.id === 2 ? <Link className="custom-kit-button" to="/custom-cctv-kit"><span>Custom CCTV Kit</span><small>Build a compatible system →</small></Link> : null}
