...
export function ProductsPage() {
  ...
  useEffect(() => {
    const query = window.matchMedia(
      "(min-width: 443px) and (max-width: 970px)"
    );
    const syncView = () => {
  ...
  return (
    <main className="page container catalogue-page">
      ...
      {categoryEntry?.category.sub_cat.length ? (
        <div className="subcategory-chips">
          {categoryEntry.category.sub_cat.map((child) => (
            <Link key={child.id} to={`/category/${child.links}`}>
              {child.title}
              <span>›</span>
            </Link>
          ))}
      ...
      <div className="catalogue-toolbar">
        ...
        <div className="view-switch" aria-label="View style">
          ...
          <button
            ...
            title={
              listDisabled
                ? "List view is unavailable at this screen width"
                : undefined
            }
          >
            ☷ List
      ...
      <div className="catalogue">
        <aside
          className={`filters catalogue-filters ${
                ...
                {filter.options.map(([id, label]) => (
                  <label key={id}>
                    <input
                      type="radio"
                      name={filter.key}
                      checked={params.get(filter.key) === String(id)}
                      onChange={() => update(filter.key, String(id))}
                    />{" "}
                    {label}
                  </label>
                ))}
