...
export function ProductDetailPage() {
  ...
  return (
    <main className="page container">
      ...
      <section className={`product-detail product-detail--commerce ${usesTiandyC36Layout ? "product-detail--tiandy-c36" : ""}`}>
      ...
      {usesTiandyC36Layout && <>
        ...
        <section className="key-features key-features--tiandy"><h2>{zh ? "主要能力" : "Capabilities"}</h2><ul>{tiandyC36Content.capabilities.map(([title, description]) => <li key={title}><strong>{title}:</strong> {description}</li>)}</ul></section>
        ...
        <section className="product-specifications"><div><h2>{zh ? "规格" : "Specifications"}</h2></div><dl>{tiandyC36Content.specifications.map(([term, detail]) => <div key={term}><dt>{term}</dt><dd>{detail}</dd></div>)}</dl></section>
