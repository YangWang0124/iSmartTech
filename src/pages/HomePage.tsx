  ...
  return <section className="home-carousel" aria-roledescription="carousel" aria-label="Store promotions">
    <div className="home-carousel__track" style={{ transform: `translateX(-${slide * 100}%)` }}>
      <article className="home-slide home-slide--installation" aria-hidden={slide !== 0}>
        <div className="container home-slide__content"><span>AUCKLAND INSTALLATION SERVICES</span><h1>Professional<span className="home-slide__mobile-break"><br /></span> Installation<br />Available<span className="home-slide__mobile-break"><br /></span> Auckland-Wide</h1><p>Get your products professionally installed by our<span className="home-slide__medium-break"><br /></span> experienced team anywhere across Auckland.</p><Link className="button button--primary" to="/installation-services">Click here to learn more about our installation services <b>→</b></Link></div>
      </article>
      <Link className="home-slide home-slide--camera-banner home-slide--alarm-banner" to="/products/hikvision-ax-pro-security-kit" aria-label="View the Hikvision AX PRO Alarm Security Kit" aria-hidden={slide !== 1}><img src="/assets/hikvision-ax-pro-banner-outpainted.png" alt="Hikvision AX PRO alarm kit in a wide natural home entrance with DS-PWA96-Kit-WB model number" loading="lazy" decoding="async" /></Link>
      <Link className="home-slide home-slide--camera-banner home-slide--dahua-banner" to="/products/dahua-4k-turret" aria-label="View the Dahua 6MP Dual Light Turret Network Camera" aria-hidden={slide !== 2}><img src="/assets/dahua-camera-banner-complete.png" alt="Dahua DH-IPC-HDW3667EM-S-IL-ANZ turret camera installed beneath a modern residential roof eave" loading="lazy" decoding="async" /></Link>
    </div>
  ...
  return (
    <>
      ...
      <section className="section container home-product-section home-category-section">
        ...
        <div className="category-grid">{homepageCategories.map(category => <Link key={category.slug} to={`/category/${category.slug}`} className="category-card"><span className="category-card__product"><img src={category.image} alt={category.product} loading="lazy" decoding="async" /></span><div><h3>{zh ? category.zhName : category.name}</h3><p>{zh ? category.zhCopy : category.copy}</p></div><b>→</b></Link>)}</div>
      </section>

      <section className="section section--tint home-product-section"><div className="container"><div className="section-heading"><div><span className="eyebrow">{zh ? "为您精选" : "OUR PICKS"}</span><h2>{zh ? "精选商品" : "Featured products"}</h2></div><Link to="/products">{zh ? "浏览全部系列" : "Browse the full range"} →</Link></div><ProductGrid products={getFeaturedProducts(products)} /></div></section>
