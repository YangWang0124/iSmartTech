...
const { alarmProducts, alarmKitComponentIds, arrowheadKitIncludedItems, arrowheadKitSupplementalItems, createAlarmDetailContent, alarmKitComponentDisplayName } = await import(`data:text/javascript;base64,${Buffer.from(js).toString("base64")}`);
const kits = alarmProducts.filter((p) => alarmKitComponentIds[p.id] || arrowheadKitIncludedItems[p.id]);
assert.equal(kits.length, 14);

for (const product of alarmProducts.filter((p) => p.brand === "Paradox" && (alarmKitComponentIds[p.id] || p.id.endsWith("control-panel")))) {
...
for (const product of kits.filter((p) => p.brand === "Arrowhead")) {
  ...
  const isLegacyAlarmWarehouseKit = product.id === "arrowhead-ec-black-touchscreen-alarm-kit";
  const expectedTokens = isLegacyAlarmWarehouseKit
    ? ["Sixteen-zone panel", "analogue dialler", "7.0Ah", "Alternative mains connection"]
    : ["32 doors", "1,900 wireless users", "13.8V", "1A", "Outputs 1 and 2", "monitored 12V", "optional", "voltage-free", "Infinity Output"];
  for (const token of expectedTokens) {
    assert(visible.includes(token), `${product.id}: missing ${token}`);
  }
  const supplemental = arrowheadKitSupplementalItems[product.id] || [];
  assert(supplemental.some(([label]) => label === "Alternative power option"), product.id + ": missing alternative power option");
  assert.equal(supplemental.some(([label]) => label === "Alternative keypad colour"), product.id.includes("-led-"));
}
